...
class TuyaAccessory extends EventEmitter {
    ...
    _connect() {
        ...
        this._socket.on('error', err => {
            this.connected = false;
            this.log.info(`Socket had a problem and will reconnect to ${this.context.name} (${this.context.version}) (${err && err.code || err})`);
            this.log.debug(`Socket error detail for ${this.context.name} (${this.context.version}): ${err && err.stack || err}`);
        ...
        this._socket.on('close', err => {
            this.connected = false;
            this.session_key = null;
            this.log.debug(`Closed connection with ${this.context.name} (${this.context.version})${err ? ' (had transmission error)' : ''}`);
        });
    ...
    _msgHandler_3_4(task, callback) {
        ...
        if (cmd === 4) { // CommandType.RENAME_GW
            ...
            this.connected = true;
            this.log.debug(`Session key negotiation succeeded for ${this.context.name} (${this.context.version}) — connected.`);
            this.update();
            this.emit('connect');
            if (this._socket._pinger)
                clearTimeout(this._socket._pinger);
    ...
    update(o) {
        ...
        if (hasDataPoint) {
            ...
            const is34or35 = this.context.version === '3.4' || this.context.version === '3.5';
            // 3.4/3.5 expect the DP_QUERY_NEW status request (command 16) with an
            // EMPTY payload ({}). Sending {gwId, devId} makes some devices reject the
            // query with "json obj data unvalid" and drop the connection (ECONNRESET),
            // causing periodic reconnects. This matches what tinytuya sends for 3.4/3.5.
            // Older protocols keep the classic cmd-10 query with the gwId/devId payload.
            result = this._send({
                data: is34or35 ? {} : {
