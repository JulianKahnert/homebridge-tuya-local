...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        // Mode command values. Accept alphabetic words (e.g. "COOL") AND numeric
        // enum strings (e.g. "1"/"3") so devices whose mode DP uses numeric enums
        // can be configured. Only reject values with unexpected characters.
        const MODE_CMD = /^[a-z0-9]+$/i;

        this.cmdCool = 'COOL';
        if (this.device.context.cmdCool) {
            if (MODE_CMD.test(this.device.context.cmdCool)) this.cmdCool = ('' + this.device.context.cmdCool).trim();
            else throw new Error('The cmdCool doesn\'t appear to be valid: ' + this.device.context.cmdCool);
        }

        this.cmdHeat = 'HEAT';
        if (this.device.context.cmdHeat) {
            if (MODE_CMD.test(this.device.context.cmdHeat)) this.cmdHeat = ('' + this.device.context.cmdHeat).trim();
            else throw new Error('The cmdHeat doesn\'t appear to be valid: ' + this.device.context.cmdHeat);
        }

        this.cmdAuto = 'AUTO';
        if (this.device.context.cmdAuto) {
            if (MODE_CMD.test(this.device.context.cmdAuto)) this.cmdAuto = ('' + this.device.context.cmdAuto).trim();
        ...
        if (!this.device.context.noRotationSpeed) {
            ...
            // Remember the step count and whether the device numbers its speeds in
            // reverse order (e.g. '1' = high, '3' = low). With fanSpeedReverse set, the
            // Tuya step value is mirrored so HomeKit's slider direction matches reality.
            this._fanSpeedSteps = fanSpeedSteps;
            this._fanSpeedReverse = !!this.device.context.fanSpeedReverse;
    ...
    _registerCharacteristics(dps) {
        ...
        this.log.debug('[AC %s] _registerCharacteristics resolved DPs: active=%s threshold=%s currentTemp=%s mode=%s rotationSpeed=%s swingMode=%s childLock=%s tempUnits=%s; incoming dps keys: %s',
            this.device.context.name,
            this.dpActive, this.dpThreshold, this.dpCurrentTemperature, this.dpMode,
            this.dpRotationSpeed, this.dpSwingMode, this.dpChildLock, this.dpTempUnits,
            Object.keys(dps || {}).join(','));
    ...
    getActive(callback) {
        this.getState(this.dpActive, (err, dp) => {
            if (err) {
                this.log.debug('[AC %s] getActive: device not ready/err (connected=%s)', this.device.context.name, this.device.connected);
                return callback(err);
            }
            this.log.debug('[AC %s] getActive: dp[%s]=%s -> %s', this.device.context.name, this.dpActive, dp, this._getActive(dp));
    ...
    setActive(value, callback) {
        ...
        this.log.debug('[AC %s] setActive: HomeKit value=%s -> writing dp[%s]=%s (connected=%s)',
            this.device.context.name, value, this.dpActive, value === Characteristic.Active.ACTIVE, this.device.connected);
    ...
    _getRotationSpeed(dps) {
        ...
        const rawTuya = dps[this.dpRotationSpeed];

        if (this._hkRotationSpeed) {
            const currntRotationSpeed = this.convertRotationSpeedFromHomeKitToTuya(this._hkRotationSpeed);

            const mapped = currntRotationSpeed === rawTuya ? this._hkRotationSpeed : this.convertRotationSpeedFromTuyaToHomeKit(rawTuya);
            this.log.debug('[AC %s] _getRotationSpeed raw=%s -> hk=%s (cached=%s)', this.device.context.name, rawTuya, mapped, this._hkRotationSpeed);
            return mapped;
        }

        this._hkRotationSpeed = this.convertRotationSpeedFromTuyaToHomeKit(rawTuya);
        this.log.debug('[AC %s] _getRotationSpeed raw=%s -> hk=%s', this.device.context.name, rawTuya, this._hkRotationSpeed);
        return this._hkRotationSpeed;
    }
    ...
    convertRotationSpeedFromTuyaToHomeKit(value) {
        // _rotationStops is built in this subclass's constructor body, which runs AFTER
        // super() — and super() registers the 'change' handler + calls _connect(). If a
        // 'change' ever fires synchronously during construction, this lookup table is still
        // undefined and `this._rotationStops[...]` would throw, crashing the child bridge.
        // Degrade gracefully to a safe default instead of throwing.
        if (!this._rotationStops) {
            this.log.warn('RotationSpeed lookup before _rotationStops initialized for %s', this.device.context.name);
            return 0;
        }
        let step = parseInt(value);
        if (this._fanSpeedReverse && step >= 1 && step <= this._fanSpeedSteps) {
            step = (this._fanSpeedSteps + 1) - step;
        }
        const mapped = this._rotationStops[step];
        this.log.debug('[AC %s] convertRotationSpeedFromTuyaToHomeKit raw=%s -> hk=%s (reverse=%s)', this.device.context.name, value, mapped, this._fanSpeedReverse);
        return mapped;
    }

    convertRotationSpeedFromHomeKitToTuya(value) {
        let step = this._rotationSteps[value];
        if (this._fanSpeedReverse && step >= 1 && step <= this._fanSpeedSteps) {
            step = (this._fanSpeedSteps + 1) - step;
        }
        return this.device.context.fanSpeedSteps ? '' + step : step;
    }
}
