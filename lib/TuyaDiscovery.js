...
class TuyaDiscovery extends EventEmitter {
  ...
  _onDgramMessage(port, msg, info) {
    ...
    try {
      ...
      // Foreign/unparseable broadcasts from other Tuya participants on the LAN
      // are expected noise — keep at debug so they don't spam the log.
      this.log.debug(
    ...
    try {
      ...
      try {
        ...
        if (result && result.gwId && result.ip) {
          ...
          // Foreign/non-device v3.5 broadcasts (e.g. the Tuya app announcing itself
          // with {"from":"app",...}) are not ours — log at debug so they don't spam.
          this.log.debug(
            `Discovery - Ignoring non-device v3.5 broadcast from ${info.address}:${port}`,
            jsonPayload
          );
        }
      } catch (ex) {
