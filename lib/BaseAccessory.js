class BaseAccessory {
    constructor(...props) {
        ...
        this.device.once('change', () => {
            ...
            this.log(`Ready to handle ${name} (${this.device.context.type}:${this.device.context.version}) with signature ${JSON.stringify(this.device.state)}`);

            // Guard the whole registration: a throw here (e.g. a DP-mapping mismatch or an
            // uninitialized lookup table) would otherwise propagate up to the EventEmitter
            // and crash the Homebridge child bridge with exit code 1, taking down every other
            // accessory in the bridge. Catch + log the full stack instead of rethrowing.
            try {
                this._registerCharacteristics(this.device.state);
            } catch (err) {
                this.log.error('Failed to register characteristics for %s: %s', name, err && err.stack || err);
            }
        });
    ...
    setMultiState(dps, callback) {
        if (!this.device.connected) {
            this.log.debug('[%s] setMultiState skipped — device not connected; dps=%s', this.device.context.name, JSON.stringify(dps));
            return callback(true);
        }
        for (const dp in dps) {
            if (dps.hasOwnProperty(dp) && dps[dp] !== this.device.state[dp]){
                this.__ret = this.device.update({[dp.toString()] : dps[dp]});
                this.log.debug('[%s] setMultiState: update dp[%s]=%s -> accepted=%s', this.device.context.name, dp, dps[dp], this.__ret);
            }
        }
    ...
    convertBrightnessFromHomeKitToTuya(value) {
        ...
        const r = Math.round(((scale - min) * value + 100 * min - scale) / 99);
